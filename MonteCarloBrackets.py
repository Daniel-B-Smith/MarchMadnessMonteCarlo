...
def energy_of_flipping(current_winner,current_loser):
    ...
    """
    return energy_game(current_loser,current_winner) - energy_game(current_winner,current_loser)
...
def simulate(ntrials,region,T,printonswap=False,showvis=True,newfig=False,teamdesc=None,printbrackets=True):
    """
    If region is "west" "midwest" "south" or "east" we'll run a bracket based just on those teams.
    If it's "all" we'll run a full bracket.
    If it's a list of teams, we'll run a bracket based just on that list.

    So, one way you might want to do things is to simulate 10000 runs for each of the four brackets,
    ...
    lb,mcb,mcb_count,unique_brackets,lowest_sightings = Stats.gather_uniquestats(brackets)
    if showvis:
        Visualization.showstats(brackets,unique_brackets,lowest_sightings,newfig=newfig,teamdesc=teamdesc)
    if printbrackets:
...
def runbracket2(ntrials1,ntrials2,T):
    ...
    for (i,region) in enumerate(regions):
        results[region] = simulate(ntrials1,region,T,newfig=i,teamdesc=region,printbrackets=False)
    # Make a new bracket from our final four
    teams = [results[region][1].bracket[-1][0] for region in regions]
    ff_lb,ff_mcb = simulate(ntrials2,teams,T,newfig=i+1,teamdesc="Final Four",printbrackets=False)
